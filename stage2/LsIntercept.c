...
int new_write(const struct pt_regs *regs);
int load(void);
void unload(void);

typedef asmlinkage int (*original_write_t)(const struct pt_regs *regs);
static unsigned long *syscall_table = (unsigned long *)0xffffffffb48001a0; 
...
asmlinkage int new_write(const struct pt_regs *regs) {
    printk(KERN_ALERT "Hijacked!");
    return original_write_ptr(regs);
}
